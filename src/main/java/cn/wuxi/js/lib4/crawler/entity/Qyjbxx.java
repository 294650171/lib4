package cn.wuxi.js.lib4.crawler.entity;

import java.io.Serializable;
import java.util.Date;

public class Qyjbxx implements Serializable {
    private String qyid;

    private String userid;

    private String qymc;

    private String zzjgdm;

    private String yyzzzch;

    private String khyh;

    private String yhzh;

    private String sfsyq;

    private Integer gcjsryZs;

    private Integer gcjsryGjzcrs;

    private Integer gcjsryZjzcrs;

    private Integer sylxid;

    private String sylx;

    private String provinceid;

    private String province;

    private String cityid;

    private String city;

    private String countyid;

    private String county;

    private String zcdd;

    private Integer jjxzid;

    private String jjxz;

    private String zczb;

    private String zyfw;

    private String jyfw;

    private Date clrq;

    private String qyjj;

    private String xxdd;

    private String yzbm;

    private String cz;

    private String email;

    private String webaddress;

    private String lxr;

    private String lxdh;

    private String fddbrRyid;

    private String fddbr;

    private String qyfzrRyid;

    private String qyfzr;

    private String cwfzrRyid;

    private String cwfzr;

    private String jsfzrRyid;

    private String jsfzr;

    private String aqfzrRyid;

    private String aqfzr;

    private Integer datastate;

    private String tag;

    private String xgr;

    private Date xgrqsj;

    private Integer xmjlzs;

    private Integer aqy;

    private Integer zjy;

    private Integer sgy;

    private String tyshxydm;

    private static final long serialVersionUID = 1L;

    public String getQyid() {
        return qyid;
    }

    public void setQyid(String qyid) {
        this.qyid = qyid == null ? null : qyid.trim();
    }

    public String getUserid() {
        return userid;
    }

    public void setUserid(String userid) {
        this.userid = userid == null ? null : userid.trim();
    }

    public String getQymc() {
        return qymc;
    }

    public void setQymc(String qymc) {
        this.qymc = qymc == null ? null : qymc.trim();
    }

    public String getZzjgdm() {
        return zzjgdm;
    }

    public void setZzjgdm(String zzjgdm) {
        this.zzjgdm = zzjgdm == null ? null : zzjgdm.trim();
    }

    public String getYyzzzch() {
        return yyzzzch;
    }

    public void setYyzzzch(String yyzzzch) {
        this.yyzzzch = yyzzzch == null ? null : yyzzzch.trim();
    }

    public String getKhyh() {
        return khyh;
    }

    public void setKhyh(String khyh) {
        this.khyh = khyh == null ? null : khyh.trim();
    }

    public String getYhzh() {
        return yhzh;
    }

    public void setYhzh(String yhzh) {
        this.yhzh = yhzh == null ? null : yhzh.trim();
    }

    public String getSfsyq() {
        return sfsyq;
    }

    public void setSfsyq(String sfsyq) {
        this.sfsyq = sfsyq == null ? null : sfsyq.trim();
    }

    public Integer getGcjsryZs() {
        return gcjsryZs;
    }

    public void setGcjsryZs(Integer gcjsryZs) {
        this.gcjsryZs = gcjsryZs;
    }

    public Integer getGcjsryGjzcrs() {
        return gcjsryGjzcrs;
    }

    public void setGcjsryGjzcrs(Integer gcjsryGjzcrs) {
        this.gcjsryGjzcrs = gcjsryGjzcrs;
    }

    public Integer getGcjsryZjzcrs() {
        return gcjsryZjzcrs;
    }

    public void setGcjsryZjzcrs(Integer gcjsryZjzcrs) {
        this.gcjsryZjzcrs = gcjsryZjzcrs;
    }

    public Integer getSylxid() {
        return sylxid;
    }

    public void setSylxid(Integer sylxid) {
        this.sylxid = sylxid;
    }

    public String getSylx() {
        return sylx;
    }

    public void setSylx(String sylx) {
        this.sylx = sylx == null ? null : sylx.trim();
    }

    public String getProvinceid() {
        return provinceid;
    }

    public void setProvinceid(String provinceid) {
        this.provinceid = provinceid == null ? null : provinceid.trim();
    }

    public String getProvince() {
        return province;
    }

    public void setProvince(String province) {
        this.province = province == null ? null : province.trim();
    }

    public String getCityid() {
        return cityid;
    }

    public void setCityid(String cityid) {
        this.cityid = cityid == null ? null : cityid.trim();
    }

    public String getCity() {
        return city;
    }

    public void setCity(String city) {
        this.city = city == null ? null : city.trim();
    }

    public String getCountyid() {
        return countyid;
    }

    public void setCountyid(String countyid) {
        this.countyid = countyid == null ? null : countyid.trim();
    }

    public String getCounty() {
        return county;
    }

    public void setCounty(String county) {
        this.county = county == null ? null : county.trim();
    }

    public String getZcdd() {
        return zcdd;
    }

    public void setZcdd(String zcdd) {
        this.zcdd = zcdd == null ? null : zcdd.trim();
    }

    public Integer getJjxzid() {
        return jjxzid;
    }

    public void setJjxzid(Integer jjxzid) {
        this.jjxzid = jjxzid;
    }

    public String getJjxz() {
        return jjxz;
    }

    public void setJjxz(String jjxz) {
        this.jjxz = jjxz == null ? null : jjxz.trim();
    }

    public String getZczb() {
        return zczb;
    }

    public void setZczb(String zczb) {
        this.zczb = zczb == null ? null : zczb.trim();
    }

    public String getZyfw() {
        return zyfw;
    }

    public void setZyfw(String zyfw) {
        this.zyfw = zyfw == null ? null : zyfw.trim();
    }

    public String getJyfw() {
        return jyfw;
    }

    public void setJyfw(String jyfw) {
        this.jyfw = jyfw == null ? null : jyfw.trim();
    }

    public Date getClrq() {
        return clrq;
    }

    public void setClrq(Date clrq) {
        this.clrq = clrq;
    }

    public String getQyjj() {
        return qyjj;
    }

    public void setQyjj(String qyjj) {
        this.qyjj = qyjj == null ? null : qyjj.trim();
    }

    public String getXxdd() {
        return xxdd;
    }

    public void setXxdd(String xxdd) {
        this.xxdd = xxdd == null ? null : xxdd.trim();
    }

    public String getYzbm() {
        return yzbm;
    }

    public void setYzbm(String yzbm) {
        this.yzbm = yzbm == null ? null : yzbm.trim();
    }

    public String getCz() {
        return cz;
    }

    public void setCz(String cz) {
        this.cz = cz == null ? null : cz.trim();
    }

    public String getEmail() {
        return email;
    }

    public void setEmail(String email) {
        this.email = email == null ? null : email.trim();
    }

    public String getWebaddress() {
        return webaddress;
    }

    public void setWebaddress(String webaddress) {
        this.webaddress = webaddress == null ? null : webaddress.trim();
    }

    public String getLxr() {
        return lxr;
    }

    public void setLxr(String lxr) {
        this.lxr = lxr == null ? null : lxr.trim();
    }

    public String getLxdh() {
        return lxdh;
    }

    public void setLxdh(String lxdh) {
        this.lxdh = lxdh == null ? null : lxdh.trim();
    }

    public String getFddbrRyid() {
        return fddbrRyid;
    }

    public void setFddbrRyid(String fddbrRyid) {
        this.fddbrRyid = fddbrRyid == null ? null : fddbrRyid.trim();
    }

    public String getFddbr() {
        return fddbr;
    }

    public void setFddbr(String fddbr) {
        this.fddbr = fddbr == null ? null : fddbr.trim();
    }

    public String getQyfzrRyid() {
        return qyfzrRyid;
    }

    public void setQyfzrRyid(String qyfzrRyid) {
        this.qyfzrRyid = qyfzrRyid == null ? null : qyfzrRyid.trim();
    }

    public String getQyfzr() {
        return qyfzr;
    }

    public void setQyfzr(String qyfzr) {
        this.qyfzr = qyfzr == null ? null : qyfzr.trim();
    }

    public String getCwfzrRyid() {
        return cwfzrRyid;
    }

    public void setCwfzrRyid(String cwfzrRyid) {
        this.cwfzrRyid = cwfzrRyid == null ? null : cwfzrRyid.trim();
    }

    public String getCwfzr() {
        return cwfzr;
    }

    public void setCwfzr(String cwfzr) {
        this.cwfzr = cwfzr == null ? null : cwfzr.trim();
    }

    public String getJsfzrRyid() {
        return jsfzrRyid;
    }

    public void setJsfzrRyid(String jsfzrRyid) {
        this.jsfzrRyid = jsfzrRyid == null ? null : jsfzrRyid.trim();
    }

    public String getJsfzr() {
        return jsfzr;
    }

    public void setJsfzr(String jsfzr) {
        this.jsfzr = jsfzr == null ? null : jsfzr.trim();
    }

    public String getAqfzrRyid() {
        return aqfzrRyid;
    }

    public void setAqfzrRyid(String aqfzrRyid) {
        this.aqfzrRyid = aqfzrRyid == null ? null : aqfzrRyid.trim();
    }

    public String getAqfzr() {
        return aqfzr;
    }

    public void setAqfzr(String aqfzr) {
        this.aqfzr = aqfzr == null ? null : aqfzr.trim();
    }

    public Integer getDatastate() {
        return datastate;
    }

    public void setDatastate(Integer datastate) {
        this.datastate = datastate;
    }

    public String getTag() {
        return tag;
    }

    public void setTag(String tag) {
        this.tag = tag == null ? null : tag.trim();
    }

    public String getXgr() {
        return xgr;
    }

    public void setXgr(String xgr) {
        this.xgr = xgr == null ? null : xgr.trim();
    }

    public Date getXgrqsj() {
        return xgrqsj;
    }

    public void setXgrqsj(Date xgrqsj) {
        this.xgrqsj = xgrqsj;
    }

    public Integer getXmjlzs() {
        return xmjlzs;
    }

    public void setXmjlzs(Integer xmjlzs) {
        this.xmjlzs = xmjlzs;
    }

    public Integer getAqy() {
        return aqy;
    }

    public void setAqy(Integer aqy) {
        this.aqy = aqy;
    }

    public Integer getZjy() {
        return zjy;
    }

    public void setZjy(Integer zjy) {
        this.zjy = zjy;
    }

    public Integer getSgy() {
        return sgy;
    }

    public void setSgy(Integer sgy) {
        this.sgy = sgy;
    }

    public String getTyshxydm() {
        return tyshxydm;
    }

    public void setTyshxydm(String tyshxydm) {
        this.tyshxydm = tyshxydm == null ? null : tyshxydm.trim();
    }
}